...
namespace IntegrationHub.SRP.Controllers
{
    ...
    public class SrpOldController : ControllerBase
    {

        private readonly PeselServiceSettings _peselSoapSettings;
        private readonly PeselServiceOld _peselService;
        private readonly ILogger<SrpOldController> _logger;

        public SrpOldController(IOptions<PeselServiceSettings> peselSoapSettings, PeselServiceOld peselService, ILogger<SrpOldController> logger)
        {
        ...
        public IActionResult CheckPeselCertificate()
        {
            ...
            #endregion
            return Ok(new ServiceResponse<ClientCertificateInfo>
            {
                IsSuccess = true,
        ...
        [HttpGet("get-person/{pesel}")]
        public async Task<IActionResult> GetPersonByPesel(string pesel, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(pesel))
            {
                ...
                {
                    IsSuccess = false,
                    Message = "Parametr PESEL jest wymagany.",
                    ResponseCode = "VALIDATION_ERROR"
                });
            }
            #endregion
            ...
            try
            {
                var client = PeselWyszukiwanieClientFactory.Create(_peselSoapSettings, cert, _logger);
                var request = new KryteriaWyszukiwaniaOsob
                {
                    numerPesel = pesel,
                };

                var requestId = Guid.NewGuid().ToString();
                var result = await client.wyszukajPodstawoweDaneOsobyAsync(requestId, request);
                return Ok(new ServiceResponse<wyszukajPodstawoweDaneOsobyResponse>
                {
                    IsSuccess = true,
                    Message = "Dane osobowe pobrane pomyślnie.",
